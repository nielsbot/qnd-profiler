...
// `ProfilerInit()` installs an `atexit()` handler and will dump the profiler
// results to file when the program exits...
...
#define ProfilerGetCurrent()                                                   \
  (struct ProfilerDataCounter *)pthread_getspecific(gProfilerDataPThreadKey)
#define ProfilerSetCurrent(__profiler_data)                                    \
  pthread_setspecific(gProfilerDataPThreadKey, (const void *)__profiler_data)

#define ProfilerStackPush()                                                    \
  struct ProfilerDataCounter *__profiler_parent = ProfilerGetCurrent();        \
  struct ProfilerDataCounter *__profiler_counter =                             \
      &__profiler_data->counters[0];                                           \
  if (__profiler_parent) {                                                     \
    while (__profiler_counter < &__profiler_data->counters[10]) {              \
      ++__profiler_counter;                                                    \
      if (__profiler_counter->parent == __profiler_parent) {                   \
        break;                                                                 \
...
#define ProfilerEnter(__function_name)                                         \
  ...
  if (!__profiler_data) {                                                      \
    __profiler_data = gProfilerTableLastUsedEntry++;                           \
    __profiler_data->name = (char *)__function_name;                           \
  }                                                                            \
  ProfilerStackPush();
...
#define PSTART                                                                 \
  ProfilerEnter(__PRETTY_FUNCTION__) @try {                                    \
    {
#define PEND                                                                   \
  }                                                                            \
  }                                                                            \
