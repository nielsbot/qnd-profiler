...
/*
 ...
 ProfilerInit() installs an atexit() handler and will dump the profiler results
 to file when the program exits...

 Define PROFILER_OUTPUT_TO_FILE if you want to append profiler output to
 "profiler_output.txt" in your app's Documents folder. You can retrieve the file
 using the Organizer in Xcode. (Click on your device, find it in the
 Applications list, turn down the disclosure triangle, and click the download
 arrow on the right.) If PROFILER_OUTPUT_TO_FILE is not defined, output written
 to stdout.
 */
...
static void ProfilerOutputTreeNode(FILE *file, struct ProfilerData *data,
                                   struct ProfilerDataCounter *counter,
                                   int indent) {
  {
    float time = (float)counter->func_time / (float)CLOCKS_PER_SEC;
    fprintf(file, "%*s%-*s %15.5f %15lu %10.5f\n", indent * 4, "",
            80 - (indent * 4), data->name, time, counter->count,
            time / (float)counter->count);
  }

  for (struct ProfilerData *testData = &gProfilerTable[0];
       testData < gProfilerTableLastUsedEntry; ++testData) {
    if (!testData->name) {
      break;
    }

    if (testData != data) {
      for (struct ProfilerDataCounter *testCounter = &testData->counters[1];
           testCounter < &testData->counters[10]; ++testCounter) {
...
void ProfilerOutputTree(FILE *file) {
  fprintf(file, "%-80s %15s %15s %10s\n", "FUNCTION", "TOTAL TIME", "COUNT",
          "AVG. TIME");

  for (struct ProfilerData *data = &gProfilerTable[0];
       data < gProfilerTableLastUsedEntry; ++data) {
...
static void ProfilerExitFunction() {
  DebugLog(@"%s\n", __PRETTY_FUNCTION__);
  ...
  @autoreleasepool {
    ...
    FILE *file = fopen(
        [[GetDocumentsFolderPath()
            stringByAppendingPathComponent:@"profiler_output.txt"] UTF8String],
        "a");
    if (!file) {
      file = stdout;
    }
...
struct ProfilerInit {
  ProfilerInit() {
    @autoreleasepool {
      ...
      {
        const struct sigaction act = {{ProfilerInterruptSignalHandler}, 0, 0};
